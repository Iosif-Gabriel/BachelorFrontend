...
export class ReviewListComponent implements OnInit{
  ...
  changesize() {
    ...
    if (reviewList) {
     reviewList.style.width='750px'
      reviewList.style.height = '260px'; 
    }
  }
  ...
  deleteFeedback(feedbackId:string){
    ...
    this.feedbackService.deleteUserFeedback(feedbackId).subscribe(
      resp => {
        ...
        this.modalService.openModal("review dell succ",this.viewContainerRef, 'Feedback deleted successfully', 'Success','Success');
