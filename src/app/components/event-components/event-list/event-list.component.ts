      ...
      this.fetchEvents().pipe(
        switchMap(events => {
          const filteredEvents = events.filter(event => {
            ...
            return eventNameLC.includes(term) || eventLocation.includes(term);
          });
  ...
  deleteEvent(eventId:string){
    ...
    this.eventService.deleteEvent(eventId).subscribe(
      resp => {
        ...
        this.modalService.openModal("event del succ",this.viewContainerRef, 'Event deleted succefully', 'Succes','Succes');
